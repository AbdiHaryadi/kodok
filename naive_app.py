...
class NaiveApp:
    def __init__(
        ...
        self.guesser = NaiveGuesser(
            object_spec_list=object_spec_list,
            inference_rules=inference_rules
        )
        self.interviewer = NaiveInterviewer(
            object_spec_list=object_spec_list,
...
if __name__ == "__main__":
    ...
    app = NaiveApp(object_spec_list, inference_rules=inference_rules)

    question_no = 0
    while (question := app.get_question()) is not None:
        guesses = app.guesser.get_all_possibly_satisfied_guesses()
        print(f"Tebakan ({len(guesses)}):", sorted(guesses, reverse=True, key=lambda x: x.satisfied), end="\n")
